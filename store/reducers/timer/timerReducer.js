import { DEFAULT, TIMER_DV } from '../../../src/constatns/timerDefaultValues';
import {
  ADD_TIMER,
  COUNT_PHASE_TIME,
  DELETE_TIMER,
...
import {
  defaultCurrTimerModel,
  defaultTimersModel,
} from '../../../src/models/Timer';
import { transformData } from '../../../src/utils/localStorage/transformData';
import { cloneDeep } from 'lodash';

const persistedState = transformData(loadData('data'));

const currTimer =
  persistedState?.currTimer && persistedState?.currTimer !== 'null'
    ? persistedState?.currTimer
    : defaultCurrTimerModel;
const timers =
  persistedState?.timers && persistedState?.timers.length > 0
  ...
  switch (action.type) {
    ...
    case TOGGLE_EDIT_TIMER:
      return {
        ...state,
        isEdit: !state.isEdit,
        editTimerData: cloneDeep(state.currTimer),
      };
    ...
    case SET_INTERVAL_COUNT:
      const val =
        action.payload === 0 ? 0 : state.intervalCount + action.payload;
